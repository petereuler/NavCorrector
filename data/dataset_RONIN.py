...
def window_dataset(gyro_data, acc_data, pos_data, ori_data, window_size=200, stride=10, filter_window=10, smooth_heading=True, heading_sigma=1, smooth_length=False, length_sigma=1.0, mode="2d"):
    """
    RONIN 数据集切窗处理 (仅 2D)
    ...
    for i, idx in enumerate(range(0, max_start, stride)):
        ...
        # 阈值判断：如果位移过小，认为静止，使用真实Yaw代替位移方向
        if displacement < 0.1: 
            center_k = (a + b) // 2
